...
mtDat <- readr::read_tsv("data/mtDNA.txt", show_col_types = FALSE) %>%
  dplyr::select(ID, Reduced) %>%
  dplyr::rename(Ind = ID, HaplogroupMT = Reduced)

datj <- dat1 %>%
  dplyr::left_join(cne_admix_rates_modernRefs, by=c("ID" = "Ind")) %>%
  dplyr::left_join(cne_admix_rates_ancientRefs, by=c("ID" = "Ind")) %>%
  dplyr::left_join(mtDat, by=c("ID" = "Ind")) %>%
...
admixed_inds_j <- admixed_inds %>%
  dplyr::left_join(source_haps, by="Haplogroup") %>%
  tidyr::drop_na()
admixed_inds_j %>%
  ggplot() + geom_point(aes(aCNE, source2_prob)) + xlim(0, 1) + ylim(0, 1)
```
...
```{r}
log_likelihood <- function(data, beta, N1, N2, adm_column = "aCNE") {
...
admixed_inds_j %>%
  ggplot() + geom_point(aes(aCNE, source2_prob)) +
...
```{r}
optimize(function(beta) log_likelihood(admixed_inds_j, beta, N1, N2, adm_column = "CNE"),
         c(0.01, 0.99), maximum = TRUE)
```

# Exploring different estimates for the admixture proportion
...
mesh <- tibble::tibble(
  ...
  likl = purrr::map_dbl(
    beta,
    ~ exp(log_likelihood(admixed_inds_j, .x, N1, N2, adm_column = "CNE"))
  )
)
norm <- sum(mesh$likl)
mesh %>% dplyr::mutate(posterior = likl / norm) %>%
